...
<p>DSO <button class="incdec_but" onclick="dsoMagUpdate(-1);">-</button><button class="incdec_but" onclick="dsoMagUpdate(1)">+</button> <i>m&leq;</i><span id="dso_level_val">5</span></p>
<p>List: <button  class="incdec_but" onclick="prevWatchList();">&lt;</button><button class="incdec_but" onclick="nextWatchList()">&gt;</button> <span id="wl_sel">None</span> <a id="list_edit_toggle" href="javascript:toggleEditWL()">[edit]</a></p>
...
<p>Lists can be selected by pressing <code>&lt;</code> and <code>&gt;' buttons in "Lists" control. When the list is selected two buttons</code>&lt;<code>and</code>&gt;` are shown in main screen under "Align" button. They allow browsing the objects forward and backward. The current selected object's name is shown under the conrols.</p>
